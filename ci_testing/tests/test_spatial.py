...
def test_Raster():
    with pytest.raises(OSError):
        ras = Raster('foobar')
    ras = Raster('ci_testing/tests/data/S1A__IW___A_20150309T173017_VV_grd_mli_geo_norm_db.tif')
    assert ras.bands == 1
    ...
    ras.write('ci_testing/tests/data/S1A__IW___A_20150309T173017_VV_grd_mli_geo_norm_db_new.tif')
    for item in finder('ci_testing/tests/data', ['S1A*_new*']):
        os.remove(item)


def test_stack():
    name = 'ci_testing/tests/data/S1A__IW___A_20150309T173017_VV_grd_mli_geo_norm_db.tif'
    outname = 'ci_testing/tests/data/S1A__IW___A_20150309T173017_VV_grd_mli_geo_norm_db_sub'
    tr = (30, 30)
    ...
    stack(srcfiles=[name, name], resampling='near', targetres=tr, overwrite=True,
          srcnodata=-99, dstnodata=-99, dstfile=outname)
    for item in finder('ci_testing/tests/data', ['S1A*_sub*']):
        os.remove(item)


def test_auxil():
    dir = 'ci_testing/tests/data/subdir'
